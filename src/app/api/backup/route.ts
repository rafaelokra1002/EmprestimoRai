...
async function buildRows(userId: string, type: BackupType) {
  if (type === "clients") {
    ...
    return clients.map((client) => ({
      ...
      Cidade: client.city || "",
      Renda: client.income != null ? fmtCurrency(client.income) : "",
      Instagram: client.instagram || "",
      "Local de Trabalho": client.workplace || "",
      Categoria: client.category || "",
      "Indicado por": client.referralName || "",
  ...
  if (type === "installment-loans") {
    return loans.map((loan) => {
      ...
      return {
        ...
        "Data Inicial": fmtDate(loan.startDate),
