...
export default function EditarEmprestimoPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f6f8f7] px-4 py-6 dark:bg-zinc-950 sm:px-6">
      <div className="mx-auto max-w-3xl rounded-[28px] border border-gray-200 bg-white shadow-[0_24px_60px_-32px_rgba(15,23,42,0.18)] dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between px-6 pb-3 pt-6 sm:px-8">
          ...
          <button
            type="button"
            onClick={() => router.push(`/emprestimos/${loanId}`)}
        ...
        <div className="max-h-[calc(100vh-96px)] space-y-6 overflow-y-auto px-6 pb-8 sm:px-8">
          ...
          <div>
            ...
            <div className="mt-3 space-y-3 max-h-[260px] overflow-y-auto rounded-2xl border border-gray-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950/20">
            {installmentDates.map((date, i) => (
              <div key={i} className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <span className="w-20 text-sm font-medium text-slate-500 dark:text-zinc-400">Parcela {i + 1}:</span>
          ...
          <div>
            ...
            {loanTags.map((tag, i) => (
              <span key={i} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-xs font-medium text-red-600 dark:bg-red-950/20 dark:text-red-300">
                {tag}
          ...
          <div className="flex justify-end gap-3 border-t border-gray-100 pt-2 dark:border-zinc-800">
            <Button variant="outline" className="rounded-2xl px-5" onClick={() => router.push(`/emprestimos/${loanId}`)}>Cancelar</Button>
